...
class ObjectInfo {
  ...
  findObjectMetadataByType (type_id) {
    return _.find(this.object_metadata.objects, (object) => {
      return (object.type_id == type_id);
    });
  }

  readObject(type) {
    var object_metadata = this.findObjectMetadataByType(type.type_id);
    this.buffer.moveTo(object_metadata.offset);

    var object_buffer = this.buffer.readBuffer(type.size);
    var data =  ObjectInfo.readType(object_buffer, type.type);

    data.raw_data_offset = object_metadata.offset;
